    ...
    _bzbSupportLib_0.setModules({
        ...
        '8': function (_exports_0) {
            const tileValueEliminate = _bzbSupportLib_0.async(function* (sudoku) {
                ...
                for (let _iterant_31 of sudoku.emptyPoints()) {
                    const point = _iterant_31;
                    const _patternPlaceholder_18 = point, _iterator_23 = _patternPlaceholder_18[Symbol.iterator](), x = _iterator_23.next().value, y = _iterator_23.next().value, box = sudoku.boxAt([
                            x,
                            y
                        ]), row = sudoku.rowAt([
                            x,
                    ...
                    for (let _iterant_32 of all) {
                        const pt = _iterant_32;
                        let _opvar_15, _opvar_16;
                        const _patternPlaceholder_19 = pt, _iterator_24 = _patternPlaceholder_19[Symbol.iterator](), i = _iterator_24.next().value, j = _iterator_24.next().value;
                        if (i === x && j === y) {
                            continue;
                        }
            ...
            const cat = _bzbSupportLib_0.require(6).cat;
        ...
        '9': function (_exports_0) {
            const hiddenSets = function (sudoku, n, collection) {
                ...
                let _opvar_17;
                if (collection === null) {
                    for (let _iterant_33 of sudoku.allCollections()) {
                        const group = _iterant_33;
                        hiddenSets(sudoku, n, group);
                    }
                } else {
                    let _opvar_18;
                    const tried = new Set(), val2points = new Map(), vals = [];
                    for (let _iterant_34 of collection.iterate()) {
                        const pt = _iterant_34;
                        const ptcode = sudoku.encode(pt);
                        if (sudoku.determined(pt)) {
                            continue;
                        }
                        for (let _iterant_35 of sudoku.getPossibilities(pt)) {
                    ...
                    for (let _iterant_36 of combinations(vals, n)) {
                        const combo = _iterant_36;
                        let _opvar_19;
                        const points = new Set();
                        for (let _iterant_37 of combo) {
                            const val = _iterant_37;
                            for (let _iterant_38 of val2points.get(val)) {
                                const ptcode = _iterant_38;
                                points.add(ptcode);
                            }
                        }
                        if (points.size === n) {
                            const comboSet = new Set(combo);
                            for (let _iterant_39 of points) {
                                const ptcode = _iterant_39;
                                const pt = sudoku.decode(ptcode);
                                if (sudoku.determined(pt)) {
                                    continue;
                                }
                                for (let _iterant_40 of sudoku.getPossibilities(pt)) {
                                    const possible = _iterant_40;
        ...
        '10': function (_exports_0) {
            const transfer = function (origin, dest) {
                    for (let _iterant_41 of dest.emptyPoints()) {
                        const pt = _iterant_41;
                    ...
                    for (let _iterant_42 of empty) {
                        const pt = _iterant_42;
                        const set = new Set();
                        for (let _iterant_43 of guesses) {
                            const guessGrid = _iterant_43;
                            let _opvar_20;
                            ...
                            _label_0:
                                for (let _iterant_44 of possible) {
                                    const possibility = _iterant_44;
                                    const tester = guessGrid.copy();
                                    tester.set(pt, possibility);
                                    for (let _iterant_45 of range(0, 2)) {
                                        const i = _iterant_45;
                                        for (let _iterant_46 of range(2, 4)) {
                                            const n = _iterant_46;
                                            hiddenSets(tester, n);
                                            tester.update();
                                        }
            ...
            const range = _bzbSupportLib_0.require(1).range;
            const tileValueEliminate = _bzbSupportLib_0.require(8).tileValueEliminate;
            const hiddenSets = _bzbSupportLib_0.require(9).hiddenSets;
            _exports_0.brutalize = brutalize;
        }
    });
    {
        const _imports_3 = _bzbSupportLib_0.require(7), fromJSON = _imports_3.fromJSON, toJSON = _imports_3.toJSON;
        const brutalize = _bzbSupportLib_0.require(10).brutalize;
        self.onmessage = function (e) {
            const _patternPlaceholder_20 = e.data, _iterator_25 = _patternPlaceholder_20[Symbol.iterator](), id = _iterator_25.next().value, _ph_0 = _iterator_25.next().value, specs = _ph_0.specs, initial = _ph_0.initial, _patternPlaceholder_21 = initial, point = _patternPlaceholder_21.point, value = _patternPlaceholder_21.value, sudoku = fromJSON(specs);
...
//# sourceMappingURL=bf-worker.bz.js.map